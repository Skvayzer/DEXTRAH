#!/usr/bin/env python3
"""Inspect a retargeted DexYCB/Revo2 trajectory and its PCA projection."""
...
def main() -> None:
    ...
    if q_retargeted.ndim != 2 or q_retargeted.shape[1] != 6:
        raise ValueError("joint_positions must have shape (frames, 6)")
    num_frames = len(q_retargeted)
    if num_frames == 0:
    ...
    if not np.allclose(
        ...
        raise ValueError("stored robot fingertips do not match the selected URDF")
    ...
    title = str(trajectory.get("capture", np.asarray(args.trajectory.stem)).item())
    mode = str(trajectory.get("grasp_mode", np.asarray("unknown")).item())
    server.add_gui_markdown(
        "## DexYCB → BrainCo Revo2\n"
        f"Capture: `{title}` · mode: `{mode}`. Human targets are orange; "
        "optimized robot fingertips are cyan. The PCA option shows the "
        "configuration reconstructed from the frozen affine subspace."
    )
    ...
    try:
        while True:
            ...
            if frame != last_frame or selected_display != last_display:
                q = q_retargeted[frame]
